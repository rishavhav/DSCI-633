...
class my_KMeans:
    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            ...
            cluster_centers = X[np.random.choice(range(len(X)))]
            ...
            for _ in range(self.n_clusters-1):
              ...
              dists = np.array([min([np.inner(center-x,center-x) for center in cluster_centers]) for x in X])
              cumulative_probs = (dists/dists.sum()).cumsum()
              r_var = np.random.rand()

              for k, v in enumerate(cumulative_probs):
                  if r_var < v:
                      i = k
                      break
    ...
    def fit_once(self, X):
        ...
        for i in range(self.max_iter+1):
            ...
            for x in X:
                ...
                # calculate inertia
                inertia += np.sum(np.square(np.amin(dists)))
